...
function resolveRelativeImageUrl(path: string, repo: any): string {
    const branch = repo?.default_branch || "main";
    // raw GitHub URL for files inside the repo
...
async function getRepoImageUrl(repo: any): Promise<string> {
    // 1) Try README first
    try {
        ...
        if (readmeRes.ok) {
            ...
            if (readmeJson?.content) {
                ...
                if (imgUrl) {
                    ...
                    if (await urlExists(finalUrl)) return finalUrl;
                }
            }
        }
    } catch {}

    // 2) Try common filenames in root via raw
    const commonNames = [
